...
def delete_ecs_cluster(stack_name):
    ...
    try:
        ...
        logger.error(f"Error deleting ECS cluster: {e}")
...
def create_ecs_cluster(stack_name):
    ...
    describe_images_response = ec2_client.describe_images(
        ...
        Filters=[{
            'Name': 'name',
            'Values': ['amzn-ami-2016.09.f-amazon-ecs-optimized']
        }]
    )
    ...
    try:
        ...
        # Ignoring error if key pair already exists
        pass
...
def create_roles(task_role_policy=None, ecs_role_policy='arn:aws:iam::aws:policy/AmazonEC2ContainerServiceFullAccess',
    ...
    try:
        ...
        logger.info(f"ECS Service Role Created: {ecs_role_arn}")
        time.sleep(1)
        ...
        logger.info(f"Task Role Created: {task_role_arn}")
        time.sleep(1)
        ...
        logger.info(f"ECS Agent Role Created: {ecsagent_role_arn}")

        role_arns = {
            'taskrolearn': task_role_arn,
            'ecsrolearn': ecs_role_arn,
            'ecsagentrolearn': ecsagent_role_arn
        }
        return role_arns
    except Exception as e:
...
def docker_login_config():
    ...
    try:
        ecr_login_token = ecr_client.get_authorization_token().get('authorizationData')[0].get('authorizationToken').encode('utf-8')
        hostname = ecr_client.get_authorization_token().get('authorizationData')[0]['proxyEndpoint'].encode('utf-8')[8:]

        home = expanduser("~")
        filename = os.path.join(home, '.docker/config.json')
        ...
        logger.info("Docker config updated successfully")
    except Exception as e:
        logger.error(f"Error during docker login configuration: {e}")
...
def setup_securitygroups_permission(ecs_security_group, elb_security_group):
    ...
    try:
        ...
        logger.info("Security group permissions updated successfully.")
    except Exception as e:
        logger.error(f"Error setting up security group permissions: {e}")


def setup(project_name='spring-petclinic-rest',
          service_list=None, region='us-west-2'):
...
def cleanup(project_name='spring-petclinic-rest',
    ...
    for target_group in target_groups:
        ...
        if target_group_name == project_name:
            ...
            logger.warning('No target group found: ' + target_group_name)
